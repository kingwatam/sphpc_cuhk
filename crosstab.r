# experimental tabulation function to replace existing tab() in helper_functions
xtab <- function(..., dec.places = NULL,
                 type = NULL,
                 style = "wide",
                 row.vars = NULL,
                 col.vars = NULL,
                 percentages = TRUE, 
                 addmargins = TRUE,
                 subtotals=TRUE){ # wrapper function for Dr Williamson's crosstab function
  
  n_args <- length(list(...))
  arg <- substitute(list(...))
  arg_values <- list(...)
  df <-  data.frame(arg_values[[1]])
  names(df)[1] <- after_dollarsign(deparse(arg[[1+1]]))
  
  if (n_args>1) {
    for (i in 2:n_args) {
      df <- cbind(df, arg_values[[i]])
      names(df)[i] <- after_dollarsign(deparse(arg[[i+1]]))
    }
  }
  
  if (is.null(row.vars)){
    row.vars <- names(df)[c(TRUE, FALSE)] # odd elements as rows e.g 1st, 3rd, 5th...
  }
  if (is.null(col.vars)){
    col.vars <- names(df)[c(FALSE, TRUE)] # odd elements as rows e.g 2nd, 4th, 6th...
  }
...
# sourced from http://pcwww.liv.ac.uk/~william/R/crosstab.r
